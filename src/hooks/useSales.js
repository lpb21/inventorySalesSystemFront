 ...
 * @param {Function} deps.loadDashboard  - Recarga datos del dashboard (incluye ventas recientes)
 * @param {Function} deps.loadCustomers  - Recarga clientes desde el backend
 */
export function useSales({ cart, cartTotal, clearCart, addToast, loadProducts, loadDashboard, loadCustomers }) {
  ...
  const processCreditSale = async (customer) => {
    ...
    try {
      ...
      await Promise.all([loadProducts(), loadDashboard(), loadCustomers()])
    } catch (error) {
