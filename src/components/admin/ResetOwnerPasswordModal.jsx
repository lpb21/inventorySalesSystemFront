...
import { X, Lock, Eye, EyeOff, RefreshCw, AlertTriangle, Copy, Check, KeyRound } from 'lucide-react'
import { useAdminTenantMutations } from '../../hooks/queries/useAdminTenants'

/**
 * ResetOwnerPasswordModal — Resetea la contraseña del propietario (owner) de un tenant.
 * Usado por el superadmin desde el panel de suscripciones.
 */
function ResetOwnerPasswordModal({ tenant, onClose, onSuccess }) {
  const { resetOwnerPassword } = useAdminTenantMutations()
  ...
  const ownerName = tenant.owner_name || tenant.business_name || tenant.name || 'Propietario'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await resetOwnerPassword.mutateAsync({ id: tenant.id, newPassword })

      setResetComplete(true)

      if (onSuccess) {
        onSuccess(`Contraseña de ${ownerName} reseteada correctamente`)
      }
    } catch (err) {
          ...
          <div className="modal-body">
            <div style={{
              ...
              <p style={{ margin: 0, fontSize: '14px', color: 'var(--success)' }}>
                La contraseña de <strong>{ownerName}</strong> ({ownerEmail}) ha sido actualizada correctamente.
              </p>
            </div>
            ...
            <div style={{
              ...
              <p style={{ margin: 0, fontSize: '13px', color: 'var(--warning)' }}>
                ...
                <strong>Importante:</strong> Comunica esta contraseña al propietario de forma segura (teléfono, WhatsApp, etc.).
              </p>
            </div>
          </div>
      ...
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px' }}>
        <div className="modal-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <h3 className="modal-title">Resetear Contraseña del Propietario</h3>
          </div>
          <button className="modal-close" onClick={onClose}>
            <X size={20} />
          </button>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            {/* Información del negocio / propietario */}
            <div style={{
              background: 'var(--surface)',
              border: '1px solid var(--border)',
              borderRadius: '8px',
            ...
            <div style={{
              ...
              <p style={{ margin: 0, fontSize: '13px', color: 'var(--warning)' }}>
                <AlertTriangle size={16} style={{ display: 'inline', marginRight: '8px', verticalAlign: 'middle' }} />
                <strong>Advertencia:</strong> Esta acción cambiará la contraseña del propietario inmediatamente.
...
export default ResetOwnerPasswordModal
